...
const getPlaceById = async (req, res, next) => {
    ...
    res.json({ place: place.toObject({ getters: true }) }) // can to write only place
...
const getPlacesByUserId = async (req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError(
            'Fetching places failed, please try again leter', 500);
        return next(error);
    }
...
const deletePlace = async (req, res, next) => {
    ...
    try {
        ...
        place.creator.places.pull(place); //מוחק את האיידיי
